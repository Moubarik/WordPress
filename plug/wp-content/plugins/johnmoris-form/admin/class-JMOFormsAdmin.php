<?php
if ( ! class_exists( 'JMOFormsAdmin' ) ) {
    class JMOFormsAdmin {
        public function __construct() {
            add_action( 'init', array( $this, 'create_post_type' ) );
            add_filter( 'manage_jmo_contact_form_posts_columns', array( $this, 'columns_head' ) );
            add_action( 'manage_jmo_contact_form_posts_custom_column', array( $this, 'columns_content' ), 10, 2 );
        }
        
        public function create_post_type() {
            register_post_type( 'jmo_contact_form',
                array(
                    'labels' => array(
                        'name' => __( 'Feedback' ),
        ...
        public function columns_head( $defaults ) {
            unset( $defaults['date'] );
            $defaults['submission_email'] = __( 'Email', 'jmo-forms' );
            $defaults['submission_website'] = __( 'Website', 'jmo-forms' );
            $defaults['submission_excerpt'] = __( 'Excerpt', 'jmo-forms' );
            $defaults['date'] = 'Date';
...
$jmoforms_admin = new JMOFormsAdmin;